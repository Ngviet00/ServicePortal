...
namespace ServicePortal.Applications.Modules.MemoNotification.Services
{
    public class MemoNotificationService : IMemoNotificationService
    {
        ...
        public MemoNotificationService (ApplicationDbContext context, IDapperQueryService dapperQueryService)
        {
            _context = context;
            _dapperQueryService = dapperQueryService;
        }

        public async Task<PagedResults<MemoNotificationDto>> GetAll(GetAllMemoNotiDto request)
        {
            ...
            var data = await _dapperQueryService.QueryAsync<MemoNotificationDto>(sql, parameters);
            var totalItems = await _dapperQueryService.QueryFirstOrDefaultAsync<int>(countSql, new { departmentId });
        ...
        public async Task<Domain.Entities.MemoNotification> Create(CreateMemoNotiDto dto, IFormFile[] files)
        {
        ...
        public async Task<Domain.Entities.MemoNotification> Update(Guid id, CreateMemoNotiDto dto, IFormFile[] files)
        {
            var memoNotify = await _context.MemoNotifications.FirstOrDefaultAsync(e => e.Id == id) ?? throw new NotFoundException("Memo notification not found!");
        ...
        public async Task<Domain.Entities.AttachFiles> GetFileDownload(Guid id)
        {
