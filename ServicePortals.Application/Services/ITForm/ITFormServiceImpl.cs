...
using File = ServicePortals.Domain.Entities.File;
...
namespace ServicePortals.Application.Services.ITForm
{
    public class ITFormServiceImpl : Interfaces.ITForm.ITFormService
    {
        ...
        public async Task<Domain.Entities.ITForm?> GetById(string applicationFormCode)
        {
            ...
            var itForm = await _context.ITForms
                ...
                .FirstOrDefaultAsync(e => e.ApplicationFormItem != null && e.ApplicationFormItem.ApplicationForm != null && e.ApplicationFormItem.ApplicationForm.Code == applicationFormCode)
            ?? throw new NotFoundException("FormIT not found, please check again");

            var files = await _context.AttachFiles
                .Where(at => at.EntityId == itForm!.Id
                          && at.EntityType == nameof(Domain.Entities.ITForm))
                .Select(at => new File
                {
                    Id = at.File!.Id,
        ...
        public async Task<object> Create(CreateITFormRequest request)
        {
            ...
            var applicationForm = new ApplicationForm
            {
                ...
                UserCodeCreatedForm = request.UserCode,
                UserNameCreatedForm = request.UserName,
                RequestTypeId = (int)RequestTypeEnum.FORM_IT,
                OrgPositionId = nextOrgPositionId ?? 9999,
            ...
            var historyApplicationForm = new HistoryApplicationForm
            {
                ...
                Action = "Created",
                UserNameAction = request?.UserName,
            ...
            var formIT = new Domain.Entities.ITForm
            {
                ...
                PriorityId = request?.PriorityId ?? 0,
            ...
            if (request?.ITCategories != null && request.ITCategories.Count > 0)
            {
            ...
            if (request?.Files.Length > 0)
            {
                ...
                foreach (var file in request.Files)
                {
                    ...
                    {
                        File = attach,
                        EntityType = nameof(Domain.Entities.ITForm),
                        EntityId = formIT.Id
                    };
            ...
            string urlApproval = $@"{_configuration["Setting:UrlFrontEnd"]}/view-form-it-approval/{applicationForm.Code}";
            ...
            BackgroundJob.Enqueue<IEmailService>(job =>
                job.SendEmailFormIT(
                    ...
                    TemplateEmail.SendContentEmail("Request for IT Form approval", urlApproval, applicationForm.Code),
                    null,
                    true
                )
            );
        ...
        public async Task<object> Delete(string applicationFormCode)
        {
            var formIT = await _context.ITForms
                ...
                ?? throw new NotFoundException("Form IT not found");
        ...
        public async Task<object> Update(string applicationFormCode, UpdateITFormRequest request)
        {
            var item = await _context.ITForms
                .FirstOrDefaultAsync(e => 
                    e.ApplicationFormItem != null && 
                    e.ApplicationFormItem.ApplicationForm != null && 
                    e.ApplicationFormItem.ApplicationForm.Code == applicationFormCode) 
            ...
            item.PriorityId = request?.PriorityId ?? 0;
            item.UpdatedAt = DateTimeOffset.Now;

            _context.ITForms.Update(item);

            if (request?.IdDeleteFile.Count > 0)
            {
                await _context.AttachFiles.Where(e => request.IdDeleteFile.Contains(e.EntityId) && e.EntityType == nameof(Domain.Entities.ITForm)).DeleteAsync();
                await _context.Files.Where(e => request.IdDeleteFile.Contains(e.Id)).DeleteAsync();
            }
        ...
        public async Task<object> Approval(ApprovalRequest request)
        {
            ...
            var itemFormIT = await _context.ITForms
                ...
                .FirstOrDefaultAsync(e => e.ApplicationFormItem != null && e.ApplicationFormItem.ApplicationFormId == request!.ApplicationFormId)
            ...
            if (request?.Status == false)
            {
                ...
                string urlView = $@"{_configuration["Setting:UrlFrontEnd"]}/view/form-it/{applicationForm.Code}";
                ...
                BackgroundJob.Enqueue<IEmailService>(job =>
                    job.SendEmailFormIT(
                        ...
                        TemplateEmail.SendContentEmail("Your request IT form has been rejected", urlView, applicationForm.Code),
                        null,
                        true
                    )
                );
            ...
            applicationForm.OrgPositionId = nextOrgPositionId ?? 9999;
            applicationForm.UpdatedAt = DateTimeOffset.Now;
        ...
        public async Task<object> AssignedTask(AssignedTaskRequest request)
        {
            ...
            string urlApproval = $@"{_configuration["Setting:UrlFrontEnd"]}/approval/assigned-form-it/{applicationForm.Code}";
            ...
            BackgroundJob.Enqueue<IEmailService>(job =>
                job.SendEmailFormIT(
                    ...
                    null,
                    "New Task Assigned",
                    TemplateEmail.SendContentEmail("New Task Assigned", urlApproval, applicationForm.Code ?? ""),
                    null,
                    true
                )
            );
        ...
        public async Task<object> ResolvedTask(ResolvedTaskRequest request)
        {
            ...
            string urlView = $@"{_configuration["Setting:UrlFrontEnd"]}/view/form-it/{itemFormIT.Id}";
            ...
            BackgroundJob.Enqueue<IEmailService>(job =>
                job.SendEmailFormIT(
                    ...
                    TemplateEmail.SendContentEmail("Your IT form request has been approved", urlView, applicationForm.Code ?? ""),
                    null,
                    true
                )
            );
