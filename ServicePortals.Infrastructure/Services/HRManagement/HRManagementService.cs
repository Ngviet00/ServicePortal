...
namespace ServicePortals.Infrastructure.Services.HRManagement
{
    public class HRManagementService : IHRManagementService
    {
        ...
        public async Task<object> GetAllHR()
        {
            var result = await _cacheService.GetOrCreateAsync($"list_hr", async () =>
            {
                ...
                var data = await _viclockDapperContext.QueryAsync<object>(sql, param);
        ...
        public async Task<object> HrAssignAttendanceManagers(HrAssignAttendanceManagersRequest request)
        {
            if (request.Data != null)
            {
                ...
                var userFromClientDict = request.Data
                    .Where(u => !string.IsNullOrWhiteSpace(u.UserCode))
                    .ToDictionary(u => u.UserCode!, u => u);

                var userManageAttendancesDict = userManageAttendances
                    .Where(u => !string.IsNullOrWhiteSpace(u.UserCode))
                    .ToDictionary(u => u.UserCode!, u => u);
                ...
                {
                    if (!string.IsNullOrWhiteSpace(item.UserCode) && !userManageAttendancesDict.TryGetValue(item.UserCode, out var dbUser))
                    {
        ...
        public async Task<List<string>> GetEmailHRByType(string type)
        {
            var result = await _context.Users
                ...
                .Where(joined => joined.HrManagements.Type == type && joined.User.Email != null)
